...
class Ads_Widget extends WP_Widget {
    ...
    function Ads_Widget() {
        ...
        $this->WP_Widget('ads_widget', 'PPO: Ads sidebar', $widget_ops, $control_ops);
    }
    ...
    function form($instance) {
        ...
        $display = '<p><label for="' . $this->get_field_id('title') . '">Title:</label>
			...
			<input   id="' . $this->get_field_id('image_url') . '" name="' . $this->get_field_name('image_url') . '" value="' . $instance['image_url'] . '" />
		</p><p>
			<label for="' . $this->get_field_id('target_url') . '">Target URL:</label>
		</p><p>	
			<input id="' . $this->get_field_id('target_url') . '" name="' . $this->get_field_name('target_url') . '" value="' . $instance['target_url'] . '" />
		</p><p>
			<label for="' . $this->get_field_id('custom_code') . '">Or Custom Code:</label>
		</p><p>	
			<textarea id="' . $this->get_field_id('custom_code') . '" name="' . $this->get_field_name('custom_code') . '">' . $instance['custom_code'] . '</textarea>
		</p>';
        print $display;
    }
